...
SIDESTEP_SPEED = 1.0
SIDESTEP_TIME  = 0.4
WAYPOINT_TIMEOUT = 15.0   # 更长一点，避免刚起飞就超时

ANGLES = [
    (-15,   0), (-15,  90), (-15, -90), (-15, 180),
    (-35,   0), (-35,  45), (-35, -45),
    ( 15,   0), ( 15,  90), ( 15, -90)
]

# ===== stall detection 参数 =====
MAX_STALL_TIME = 5.0     # 容忍卡死时间（秒）
STALL_VEL_THRESH = 0.3   # 速度阈值（m/s）
...
def save_sample(drone: str, tag: str, img: np.ndarray, depth: np.ndarray,
    ...
    meta = {"drone": drone, "pitch_deg": pitch, "yaw_deg": yaw,
...
# 避障（简化版）
# ===============================
def lidar_forward_obstacle(client: airsim.MultirotorClient, drone: str,
                           fov_deg: float = 20.0) -> float:
    try:
        data = client.getLidarData(vehicle_name=drone)
        ...
        if pts.size == 0:
            return float("inf")
        ang = np.abs(np.degrees(np.arctan2(pts[:, 1], pts[:, 0])))
        forward = pts[(pts[:, 0] > 0) & (ang < fov_deg)]
        if forward.size == 0:
            return float("inf")
        return float(np.min(np.linalg.norm(forward, axis=1)))
    except Exception:
        return float("inf")
...
# 多角度拍摄
# ===============================
def capture_at_waypoint(client: airsim.MultirotorClient, drone: str, tag: str):
    for (pitch, yaw) in ANGLES:
        ...
        if img is not None and depth is not None:
            save_sample(drone, tag + f"_p{int(pitch)}_y{int(yaw)}", img, depth, pitch, yaw)

# ===============================
# 无人机线程（带 stall detection）
# ===============================
    ...
    try:
        for i, (x, y, z) in enumerate(points):
            ...
            while True:
                ...
                log.debug(f"[{drone}] wp#{i} moving... pos=({pos.x_val:.1f},{pos.y_val:.1f},{pos.z_val:.1f}), speed={speed:.2f}")
                ...
                if speed < STALL_VEL_THRESH:
                    if stall_t0 is None:
                        ...
                        log.warning(f"[{drone}] wp#{i} stalled for {MAX_STALL_TIME}s, skipping...")
                        skipped_points += 1
                        break
                else:
                    stall_t0 = None
            ...
            img, depth = get_rgb_depth(client, drone)
            if img is not None and depth is not None:
                save_sample(drone, tag, img, depth, 0, 0)
                captured_sets += 1
